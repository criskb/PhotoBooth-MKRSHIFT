    ...
    if (hostedWorkflowApi) {
      fetchHostedRunStatus(requestComfyUrl, promptId)
        .then(async (runStatus) => {
          ...
          let outputUrl = initialOutputUrl;
          ...
          if (localHostedOutputUrl) {
            outputUrl = localHostedOutputUrl;
            responsePayload.previewUrl = localHostedOutputUrl;
          }
          responsePayload.outputUrl = outputUrl;
          const hasVerifiedOutput = Boolean(localHostedOutputUrl || outputSaved.has(promptId));
          responsePayload.complete = failed ? true : complete && hasVerifiedOutput;
          if (complete && !responsePayload.complete) {
            responsePayload.label = "Finalizing";
          }
