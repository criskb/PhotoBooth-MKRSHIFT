...
def _load_workflow_styles(workflow_dir: str) -> dict[str, None]:
    ...
    for fname in os.listdir(workflow_dir):
        ...
        if name.startswith("default") or "_save" in name:
            continue
        styles.append(name)
    return {name: None for name in sorted(styles)}


dico_styles = _load_workflow_styles(COMFY_WORKFLOW_DIR) or {"default": None}
